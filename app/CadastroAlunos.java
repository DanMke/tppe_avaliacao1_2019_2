...
//		}
	...
	public Aluno[] ordenarAlunos() {
		...
		for (int i = 0; i < resposta.length - 1; i++) {
			for (int j = i; j < resposta.length; j++) {
				String matricula1 = getMatriculaFormatada(resposta[i]);
				String matricula2 = getMatriculaFormatada(resposta[j]);
	...
	private String getMatriculaFormatada(Aluno aluno) {
		return aluno.getPrefixoMatricula() + "/" + aluno.getSufixoMatricula();
	}
